...
if ($_POST) {
    ...
    $or_servi_vo->setCli_id($_POST["inpTdCli"]);
    $or_servi_vo->setCli_docum($_POST["inpNumCli"]);
    ...
    if ($or_servi_dao->insertarOrden_serv($or_servi_vo) == 1) {
        ...
        $json_id_os = json_encode($or_servi_dao->consulta_datos_UltimaOS_cl_suc($or_servi_vo->getCli_id(), $or_servi_vo->getCli_docum()));

        if (!empty($json_id_os)) {
            $os_id = json_decode($json_id_os);
            $es_x_serv_vo->setOrden_id($os_id[0]->os_id);
            $_SESSION["num_os_cli_suc"] = $os_id[0]->os_id;
            $_SESSION["dir_os_cli_suc"] = $os_id[0]->os_direccion;
            $_SESSION["ciu_os_cli_suc"] = $os_id[0]->ciu_nombre;
            $_SESSION["tipe_os_cli_suc"] = $os_id[0]->te_desc;
            $_SESSION["tips_os_cli_suc"] = $os_id[0]->ts_desc;
            $_SESSION["tiped_os_cli_suc"] = $os_id[0]->te_id;
